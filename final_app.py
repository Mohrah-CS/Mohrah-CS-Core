...
st.set_page_config(
    page_title="MOHRAH CS CORE - Multi-Subject Edition",
    layout="wide",
    page_icon="💎",
...
with st.sidebar:
    st.title("💎 MOHRAH CS CORE")
    st.markdown("---")
    
    # اختيار المادة الرئيسية
    main_subject = st.selectbox("Select Subject / اختر المادة", ["Theory of Computation", "Operating Systems", "Community & Contact"])
    
    if main_subject == "Theory of Computation":
        subject = st.selectbox("Select Lesson / اختر الدرس", [
            "Home Page", "Foundations of TOC", "DFA Explorer", "NFA Masterclass", 
            "Regular Expressions", "DFA to RE & Pumping Lemma", 
            "CFG & Chomsky Form", "PDA & CFL Theory", 
            "Turing Machines & Algorithms", "🎓 Course Completion"
        ])
        ...
        subject = st.selectbox("Select Option:", ["Contact Developer", "Community Feedback"])
...
if main_subject == "Theory of Computation":
    ...
    if subject == "Introduction to OS":
        st.markdown("## ⚙️ Operating Systems: Introduction")
        tab_info, tab_viz, tab_q = st.tabs(["📖 Detailed Lesson", "🎨 OS Architecture", "📝 Quiz"])
        with tab_info:
            st.markdown("""
            <div class="learning-card">
            <div class="concept-badge">Module 1.1</div>
            ...
            <ul>
                <li><b>Batch OS:</b> Executes jobs in batches without user interaction.</li>
                <li><b>Time-Sharing OS:</b> Allows multiple users to share the system simultaneously.</li>
                <li><b>Real-Time OS:</b> Used in environments where rigid time requirements are present.</li>
                <li><b>Distributed OS:</b> Manages a group of independent computers as a single system.</li>
            </ul>
            </div>
            """, unsafe_allow_html=True)
        with tab_viz:
            st.markdown("### Visual: OS Abstract View")
            os_diag = graphviz.Digraph()
            os_diag.node('User', 'Users', shape='person')
            os_diag.node('App', 'Applications / Software', shape='box')
            os_diag.node('OS', 'Operating System', style='filled', fillcolor='lightblue')
            os_diag.node('HW', 'Hardware (CPU, RAM, I/O)', shape='box', style='filled', fillcolor='lightgrey')
            os_diag.edge('User', 'App')
            os_diag.edge('App', 'OS')
            os_diag.edge('OS', 'HW')
            st.graphviz_chart(os_diag)
        with tab_q:
            st.markdown("### 📝 OS Introduction Quiz (10 Questions)")
            os_qs = [
                ("What is the primary goal of an OS?", ["Convenience & Efficiency", "Playing Games", "Only Hardware"], "Convenience & Efficiency"),
                ("Which part of OS manages the CPU?", ["Process Management", "File Management", "I/O Management"], "Process Management"),
                ("An OS is a type of:", ["System Software", "Application Software", "Hardware"], "System Software"),
                ("Which OS type is used for rigid time requirements?", ["Real-Time OS", "Batch OS", "Network OS"], "Real-Time OS"),
                ("The 'Kernel' is:", ["The core part of OS", "A user application", "A hardware device"], "The core part of OS"),
                ("GUI stands for:", ["Graphical User Interface", "General User Input", "Global Unit Interface"], "Graphical User Interface"),
                ("Which function handles RAM allocation?", ["Memory Management", "Storage Management", "Security"], "Memory Management"),
                ("A 'Process' is:", ["A program in execution", "A static file", "A hardware component"], "A program in execution"),
                ("Multi-tasking means:", ["Running multiple tasks at once", "One task at a time", "No tasks"], "Running multiple tasks at once"),
                ("Which is NOT an OS?", ["Google Chrome", "Windows 11", "Linux"], "Google Chrome")
            ]
            os_score = 0
            for i, (q, opts, ans) in enumerate(os_qs):
                u_ans = st.radio(f"{i+1}. {q}", opts, key=f"osq_u_{i}")
                if u_ans == ans: os_score += 1
            if st.button("Submit OS Quiz"):
                st.success(f"Your Score: {os_score}/10")
    else:
        st.info(f"The module '{subject}' is currently under development. Stay tuned!")
