...
if subject == "Home Page":
    ...
    with tab_intro:
        ...
        <p>This branch studies abstract machines and the computational problems that can be solved using these machines. Key concepts include Finite Automata (DFA, NFA), Pushdown Automata (PDA), and Turing Machines.</p>
        <h4>2. Computability Theory</h4>
        <p>This branch deals with the fundamental question of what problems can be solved algorithmically. It explores the limits of computation, identifying problems that are 'computable' and those that are 'uncomputable'.</p>
        <h4>3. Complexity Theory</h4>
        <p>This branch focuses on the resources (time and space) required to solve computational problems. It classifies problems based on their inherent difficulty.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_alphabets:
        ...
        <p>An <b>Alphabet (Σ)</b> is a finite, non-empty set of symbols. These symbols are the basic building blocks from which all strings and languages are constructed.</p>
        <h4>Examples:</h4>
        <ul>
    ...
    with tab_strings:
        ...
        <ul>
            <li><b>Length (|w|):</b> The number of symbols in a string.</li>
            <li><b>Empty String (ε):</b> A unique string with length 0.</li>
            <li><b>Concatenation:</b> Joining two strings together.</li>
            <li><b>Reverse (wᴿ):</b> Writing symbols in reverse order.</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_languages:
        ...
        <p>A <b>Language (L)</b> over an alphabet Σ is a subset of Σ*. Languages can be finite or infinite.</p>
        <h4>Key Concepts:</h4>
        <ul>
            <li><b>Σ* (Kleene Closure):</b> The set of all possible strings over Σ, including ε.</li>
            <li><b>Σ⁺ (Positive Closure):</b> Σ* excluding the empty string ε.</li>
            <li><b>Empty Language (∅):</b> A language that contains no strings.</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_sets:
        st.markdown("""
        <div class="learning-card">
        <h3>Set Theory in TOC</h3>
        <p>Sets are collections of distinct objects. We use them to define states, alphabets, and languages.</p>
        <div class="info-grid">
            <div class="info-item"><b>Union (A ∪ B):</b> Elements in A or B.</div>
            <div class="info-item"><b>Intersection (A ∩ B):</b> Elements in both A and B.</div>
            <div class="info-item"><b>Difference (A - B):</b> Elements in A but not in B.</div>
            <div class="info-item"><b>Power Set P(S):</b> Set of all subsets of S.</div>
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_functions:
        st.markdown("""
        <div class="learning-card">
        <h3>Functions in Automata</h3>
        <p>A <b>Function (f: A → B)</b> assigns each element in set A to exactly one element in set B.</p>
        <ul>
            <li><b>Transition Function (δ):</b> Maps (Current State × Input Symbol) → Next State.</li>
            <li><b>Total Function:</b> Defined for every element in its domain (Required for DFA).</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_boolean:
        ...
        <div class="info-grid">
            <div class="info-item"><b>AND (∧):</b> True only if both are True.</div>
            <div class="info-item"><b>OR (∨):</b> True if at least one is True.</div>
            <div class="info-item"><b>NOT (¬):</b> Inverts the value.</div>
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_info:
        ...
        <p>A <b>DFA</b> is formally defined as a 5-tuple (Q, Σ, δ, q₀, F).</p>
        <ul>
            <li><b>Q:</b> Finite set of states.</li>
            <li><b>Σ:</b> Finite alphabet.</li>
            <li><b>δ:</b> Transition function (Q × Σ → Q).</li>
            <li><b>q₀:</b> Start state.</li>
            <li><b>F:</b> Set of accept states.</li>
        </ul>
        <div class="step-box">
        <b>💡 Acceptance Process:</b>  

        1. Start at q₀.  

        2. Read input symbol by symbol.  

        3. Follow unique transitions.  

        4. If final state is in F after last symbol, string is <b>Accepted</b>.
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_info:
        ...
        <p>An <b>NFA</b> is a finite automaton where for each state and input symbol, there can be zero, one, or more than one next state.</p>
        <ul>
            <li><b>ε-transitions:</b> Allowed transitions on the empty string.</li>
            <li><b>Acceptance:</b> Accepted if <i>at least one</i> path leads to an accept state.</li>
            <li><b>Equivalence:</b> NFAs recognize the same class of languages as DFAs (Regular Languages).</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)

    with tab_viz:
        st.markdown("#### NFA accepting strings containing '101'")
    ...
    with tab_ops:
        ...
        <p>Regular Operations are used to build complex languages from simpler ones.</p>
        <div class="info-grid">
            <div class="info-item"><b>1. Union (L₁ ∪ L₂):</b> Strings in either L₁ or L₂.</div>
            <div class="info-item"><b>2. Concatenation (L₁ ∘ L₂):</b> String from L₁ followed by string from L₂.</div>
            <div class="info-item"><b>3. Star (L*):</b> Zero or more strings from L.</div>
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_re:
        ...
        <p>A <b>Regular Expression</b> is a concise way to describe a regular language.</p>
        <ul>
            <li><b>a</b> : symbol from Σ.</li>
            <li><b>ε</b> : empty string.</li>
            <li><b>∅</b> : empty language.</li>
            <li><b>R₁ ∪ R₂</b> : Union.</li>
            <li><b>R₁ ∘ R₂</b> : Concatenation.</li>
            <li><b>R₁*</b> : Kleene Star.</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_closure:
        ...
        <p>Regular Languages are <b>closed</b> under:</p>
        <div class="info-grid">
            <div class="info-item">✅ Union</div>
            <div class="info-item">✅ Concatenation</div>
            <div class="info-item">✅ Star</div>
            <div class="info-item">✅ Complement</div>
            <div class="info-item">✅ Intersection</div>
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_conv:
        ...
        <p>Every RE can be converted into an equivalent NFA by building small NFAs and combining them.</p>
        <div class="step-box">
        <b>Steps:</b>  

        1. Base cases (a, ε, ∅).  

        2. Union: New start with ε-transitions.  

        3. Concatenation: Connect final to start.  

        4. Star: Loop back with ε-transitions.
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_info:
        ...
        <p>A <b>PDA</b> is a Finite Automata with an added <b>Stack</b> (LIFO memory).</p>
        <h4>The 7-Tuple Definition:</h4>
        <p>PDA = (Q, Σ, Γ, δ, q0, Z0, F)</p>
        <div class="step-box">
        <b>Acceptance Criteria:</b>  

        - <b>Final State:</b> Ends in q ∈ F.  

        - <b>Empty Stack:</b> Finishes input and stack is empty.
        </div>
        </div>
        """, unsafe_allow_html=True)
